...
def calculate_optical_flow(year, ds, cfg):
    ...
    with open(
        f"F:/eumetsat/2016/process/optical_flow/{year}.npy", "wb"
    ) as f:
...
@hydra.main(
    version_base="1.2", config_path=root / "configs", config_name="calculate_optial.yaml"
)
def main(cfg: DictConfig) -> None:
    ds = xr.open_zarr(cfg.dataset_path)
