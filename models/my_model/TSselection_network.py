...
class Linear_Residual_Unit(nn.Module):
    def __init__(
        self,
        input_size: int,
        hidden_size: int,
...
class Weather_feather_Selection_Unit(nn.Module):
    def __init__(
        self,
        ...
        dropout: float = 0.1,
        context_size: int = None,
        single_variable_grns: Dict[str, Linear_Residual_Unit] = {},
        ...
        if self.num_inputs > 1:
            if self.context_size is not None:
                self.flattened_grn = Linear_Residual_Unit(
        ...
        for name, input_size in self.input_sizes.items():
            if name in single_variable_grns:
                ...
                self.single_variable_grns[name] = Linear_Residual_Unit(
                    input_size,
