...
from data_provider.utiles import calculate_possible_starts
...
class TSContextDataset(Dataset):
    ...
    def get_channel_ids(
        ...
        if self._optflow_channel_ids is None and optflow_tensor is not None:
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        optflow_tensor = None
        optflow_path = f"{year}/ctx_opt_flow/data"
        if optflow_path in self.deeplake_ds.tensors:
            optflow_tensor = self.deeplake_ds[optflow_path]

        context_channel_ids, optflow_channel_ids, ts_channel_ids = self.get_channel_ids(context_tensor, optflow_tensor, ts_tensor)
        ...
        if optflow_tensor is not None:
            return_tensors["optical_flow"] = optflow_data
        # print("context_data", context_data.shape)

        # print("coords", coords.shape)
        # print("station_elevation", station_elevation.shape)
        # print("station_coords", station_coords.shape)
        if self.use_target:
            return_tensors["target"] = target
            # print("station_coords", return_tensors["target"].shape)
